...
def run_lstm(
    ...
    print("SingleTower LSTM", "Using subword" if use_subword else "Using main word", f"Word vector: {word_vec_path}, Device: {device}, LR={LR}, Repeat={repeat}")
    ...
    class SingleTowerLSTM(nn.Module):
        ...
        def forward(self, x, lengths):
            ...
            logits = self.fc(h_avg).squeeze(1)
            return logits

    def train_one_epoch(model, dataloader, criterion, optimizer):
        model.train()
        total_loss = 0
        for seqs, labels, lengths in tqdm(dataloader, desc="Training", disable=not show_progress):
            optimizer.zero_grad()
    ...
    def evaluate(model, dataloader):
        ...
        preds, trues = [], []
        with torch.no_grad():
            for seqs, labels, lengths in tqdm(dataloader, desc="Testing", disable=not show_progress):
                logits = model(seqs, lengths)
    ...
    for run in range(1, repeat + 1):
        ...
        model = SingleTowerLSTM(embedding_matrix, hidden_size=HIDDEN_SIZE, dropout=DROPOUT, avg_tokens=avg_tokens).to(device)
        ...
        if savemodel:
            ...
            model_path = os.path.join(model_dir, f"lstm_run_{run}.pt")
