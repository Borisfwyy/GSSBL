...
def run_usif(word_vec_path: str, data_paths: list, output_csv_path: str, repeat: int = 5, cuda_device:int = 0, use_subword: bool = False, lr = 1e-4):
    EMBED_DIM = 512
    SMOOTHING_A = 1e-3
    threshold = 0.5
    PATIENCE = 100
    ...
    with open(output_csv_path, "w", encoding="utf-8", newline="") as f:
        ...
        for name in model_dict.keys():
            ...
            if name == "MLP":
                ...
                writer.writerow([
                    name, 1,
                    f"{results['auroc'][0]:.8f}",
                    f"{results['aupr'][0]:.8f}",
                    f"{results['acc'][0]:.8f}",
                    f"{results['precision'][0]:.8f}",
