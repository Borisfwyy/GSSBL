...
def saveresult_weighted_fusion_siamese(
    train_path,
    test_path,
    ...
    print(f"vector file: {word_vec_path_meaning} and {word_vec_path_shape}, subword: {use_subword}, epochs: {EPOCHS}, lr: {LR}, batchsize: {BATCH_SIZE}, hidden_size: {HIDDEN_SIZE}, embed_dim: {EMBED_DIM}")
    ...
    def evaluate(model, loader, threshold=0.5):
        ...
        for s1, s2, label in loader.dataset.data:
            s1_texts.append(s1)
            s2_texts.append(s2)
        ...
        details = list(zip(s1_texts, s2_texts, trues, preds))
        return auroc, aupr, acc, precision, recall, f1, details
    ...
    auroc_list, aupr_list, acc_list, precision_list, recall_list, f1_list = [], [], [], [], [], []
    ...
    for run in range(repeat):
        ...
        auroc, aupr, acc, precision, recall, f1, details = evaluate(model, test_loader)

        scores = np.array([d[3] for d in details])
        ranks = scores.argsort()[::-1]  
        percentiles = np.zeros_like(scores)
        for rank, idx in enumerate(ranks):
            percentiles[idx] = 100 * (1 - rank / len(scores))
        save_path = output_csv_path.replace(".csv", f"_repeat{run+1}_details.csv")
        with open(save_path, "w", newline="", encoding="utf-8") as f_detail:
            writer = csv.writer(f_detail)
 
            writer.writerow(["s1", "s2", "label", "score", "percentile"])
            for (s1, s2, label, score), pct in zip(details, percentiles):
                writer.writerow([s1, s2, label, f"{score:.10f}", f"{pct:.3f}"])
        ...
        with open(output_csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([run + 1, auroc, aupr, acc, precision, recall, f1])
