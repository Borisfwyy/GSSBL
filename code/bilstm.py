...
def run_siamese(
    ...
    EPOCHS = 100
    FEATURE_MODE = "concat"
    print("Bilstm", "使用子字" if use_subword else "使用主字", f"词向量: {word_vec_path}, 设备: {device}, LR={LR}")
    # 加载词向量
    ...
    def train_one_epoch(model, dataloader, criterion, optimizer):
        model.train()
        total_loss = 0
        for s1, s2, labels, len1, len2 in tqdm(dataloader, desc="训练"):
            optimizer.zero_grad()
    ...
    def evaluate(model, dataloader):
        ...
        preds, trues = [], []
        with torch.no_grad():
            for s1, s2, labels, len1, len2 in tqdm(dataloader, desc="测试"):
                logits = model(s1, s2, len1, len2)
